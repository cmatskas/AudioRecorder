...
public struct Utterance: Identifiable, Codable, Equatable, Sendable {
    ...
    /// Wall-clock time the utterance ended.
    public let timestamp: Date

    public init(id: UUID = UUID(), speaker: Speaker, text: String, timestamp: Date) {
        self.id = id
...
public protocol InsightsPipeline: AnyObject, Sendable {
    ...
    /// Begins streaming/analysis. `sessionDirectory` is where transcript.json
    /// and insights.json are persisted alongside the audio masters.
    @MainActor func start(sessionDirectory: URL?)
